import { ANCIENS_PREFIXES_MTN, INDICATIF, LONGUEUR_ANCIEN, PREFIXES } from './plan.js';
...
export interface Ligne {
  ...
  source: 'prefixe' | 'ancien-prefixe-mtn' | 'inconnu';
}
 ...
 *
 * Pour un ancien numéro à 8 chiffres, l'information n'est pas dans le numéro.
 * Seule MTN publie la liste de ses anciens préfixes, ce qui permet de trancher
 * pour elle. Pour le reste, on renvoie `null` — et c'est délibéré : deviner
 * ferait passer un paiement Mobile Money chez le mauvais opérateur, ou envoyer
 * un SMS via la mauvaise passerelle. Une réponse « je ne sais pas » se traite ;
...
export function operateur(entree: unknown): Ligne {
  ...
  if (n.length === LONGUEUR_ANCIEN) {
    if (ANCIENS_PREFIXES_MTN.includes(n.slice(0, 2))) {
      return { operateur: 'mtn', type: 'mobile', source: 'ancien-prefixe-mtn' };
    }
    // Orange et Moov ne publient pas leurs anciens préfixes : on ne devine pas.
    return { operateur: null, type: null, source: 'inconnu' };
  }
